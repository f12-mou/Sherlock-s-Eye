...
async function getGroupPostsSortedByNewest(user_id, group_id){
    const sql =`SELECT GP.*, U.USER_ID, U.FIRST_NAME||' '|| U.LAST_NAME USER_NAME,  G.ADMIN_ID,
...
// async function searchGroupPostsLikeBlogs(user_id, str){
//     const sql =`SELECT B.*, U.FIRST_NAME|| ' '|| U.LAST_NAME USER_NAME,
...
//                 FROM GROUP_POSTS B LEFT OUTER JOIN USERS U ON B.USER_ID = U.USER_ID
//                 WHERE U.FIRST_NAME|| ' '|| U.LAST_NAME LIKE UPPER('%'||:str||'%')
